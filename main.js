...
// --- متغيرات خدمات Firebase (سيتم تهيئتها لاحقًا) ---
let db, auth;
...
async function main() {
    ...
    onAuthStateChanged(auth, (user) => {
        if (user) {
            if (user.isAnonymous) {
                isAdmin = false;
                adminPanelBtn.classList.remove('admin-active');
    ...
    document.addEventListener('click', (e) => {
        if (controlsMenu.classList.contains('active') && !mobileMenuTrigger.contains(e.target) && !controlsMenu.contains(e.target)) {
    ...
    // [FIX] Event delegation for all suggestion buttons
...
function addMessage(message, isUser = false, imageSrc = null, shouldScroll = true) {
    ...
    if (isUser) {
        ...
        const isCourseListCheck = (message.includes('**---') || message.includes('مرحلة')) && message.includes('**-') && message.includes('- ');
        if (message.includes('|') || isCourseListCheck) {
            try {
...
function updateSuggestions(contextText = '') {
    suggestionsContainer.innerHTML = '';
    ...
    Array.from(finalSuggestions).slice(0, 5).forEach(text => {
        ...
        // The event listener is now handled by delegation in the main() function
        suggestionsContainer.appendChild(btn);
    });
}

function showWelcome() {
    const welcomeMessage = `أهلاً بيك مع المساعد الذكي ل<span class="highlight-text">جامعة حلوان التكنولوجية</span>! الموقع غير قابل للاختراق — حتى الـFBI جرّب وماقدروش 😎. لو عندك استفسار أو مشكلة بلغنا، لأنك مش هتعرف تهكرنا ❤️`;
...
async function speakText(text, buttonElement) {
    if (!db || !auth) {
        console.error("Firebase not initialized. Cannot update audio count.");
        return;
    }
    ...
    if (audioUrl) {
        const statsRef = doc(db, "stats", "counters");
        playAudio(audioUrl, buttonElement, statsRef);
    }
}

async function playAudio(audioUrl, buttonElement, statsRef) {
